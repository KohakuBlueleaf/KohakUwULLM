"""TIPO on Kohaku-MoE-3B over 4 pipeline stages: 2907M total, 617M active.

The rung that cannot run DDP at all -- ~47 GB of bf16 weights plus fp32 moments
against a 32 GB card -- so pipelining is the enabler, not an optimization.
See docs/internals/pipeline.md.

Run:
    kogine run scripts/train/lm.py --config configs/lm/tipo/tipo_moe_3b_pp4.py
...
# The stage owns precision through PIPELINE_KWARGS, and num_microbatches is the
# accumulation; both are checked at startup.
PRECISION = "32-true"
GRAD_ACC = 1
GRAD_CLIP = 1.0
EPOCH = 1
