"""Microbatch sweep: 32768 x 8 with MXFP8, 1F1B.

The widest microbatch the card can hold: fixed state is ~2.7 GiB and activations
~0.81 GiB per 1024 tokens, so this projects to ~29.3 of 31.36 GiB. Eight
microbatches puts the 1F1B bubble at 27.3%. Sixty steps, no checkpoint.

    kogine run scripts/train/lm_pipe.py --config configs/lm/sweep/sweep_32768x08_mxfp8.py
"""
