"""Microbatch sweep: 16384 x 16 with MXFP8 on the DualPipeV schedule.

Two chunks per rank in a V, so rank 0 holds the first and last chunks and the
warmup depths sum equally. Sixty steps, no checkpoint, no preview.

    kogine run scripts/train/lm_pipe.py --config configs/lm/sweep/sweep_16384x16_dualpipev.py
"""
